...
@pytest.mark.asyncio
@respx.mock
async def test_search_records_warns_on_search_cap(tool_context) -> None:
    ...
    mcp = register_module(records_read, tool_context)
    out = await call(mcp, "accela_search_records")(limit=100)
...
async def test_get_my_records(tool_context) -> None:
    respx.get("https://apis.test.example/v4/records/mine").mock(
        return_value=Response(
            200,
            json={"result": [{"id": "X"}], "page": {"offset": 0, "limit": 25, "hasmore": False}},
        )
    )
    mcp = register_module(records_read, tool_context)
